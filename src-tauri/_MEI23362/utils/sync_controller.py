...
Implements delta sync, conflict resolution, and batch operations
"""
...
class ConflictResolver:
    ...
    def resolve_by_rule(table_name: str, local_record: Dict, mysql_record: Dict) -> Dict:
        ...
        # Bills and BillItems: Local always wins
...
class SyncController:
    ...
    def push_sync_with_validation(self, sync_data: Dict[str, List[Dict]], validate_users: bool = True) -> Dict[str, Any]:
        """
        Push sync with optional user validation (alias for backward compatibility)
        """
        if validate_users:
            return self.push_sync(sync_data)
        else:
            # Call the original push_sync logic without validation
            return self._push_sync_without_validation(sync_data)

    def _push_sync_without_validation(self, sync_data: Dict[str, List[Dict]]) -> Dict[str, Any]:
        """
        Original push_sync without user validation for bills
        """
        logger.info("Starting push_sync WITHOUT user validation")
        connection = None
        results = {
            'success': False,
            'synced_ids': {},
        ...
        try:
            ...
            connection.commit()
            results['success'] = True
            logger.info(f"push_sync (no validation) completed: {results['stats']['synced']} records synced")

        except Exception as e:
            if connection:
                connection.rollback()
            logger.error(f"Error during push_sync (no validation): {e}")
    ...
    def get_sync_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'last_sync': last_sync.get('timestamp').isoformat() if last_sync and last_sync.get('timestamp') else None,
                'last_sync_type': last_sync.get('change_type') if last_sync else None
            }
