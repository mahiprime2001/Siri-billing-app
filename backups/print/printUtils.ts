...
export async function openPrintWindow(htmlContent: string, paperSize: string): Promise<void> {
  ...
  printTimeout = setTimeout(() => {
    ...
    try {
      ...
      resizePrintFrameToContent(printFrame);
...
async function createPrintFrame(paperSize: string): Promise<HTMLIFrameElement> {
  return new Promise((resolve, reject) => {
    ...
    iframe.style.cssText = `
      ...
      height: 1px!important; /* Keep tiny; print size is controlled by @page in content */
...
/** Resize iframe to match content height (helps avoid extra blank space) */
function resizePrintFrameToContent(iframe: HTMLIFrameElement): void {
  try {
    const doc = iframe.contentDocument!;
    ...
    // No-op: if we can't measure, print will still work
  }
}
    ...
    <head>
      ...
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        html, body { font-family:Arial,sans-serif; font-size:12px; line-height:1.4; background:#fff; height:auto; }
        @page { margin: 0; }
        @media print {
          html, body { margin:0!important; padding:0!important; height:auto!important; -webkit-print-color-adjust:exact!important; }
