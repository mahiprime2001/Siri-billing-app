...
def get_or_create_customer(customer_name: str, customer_phone: str, customer_email: str = '', customer_address: str = '') -> str:
    ...
    if not supabase:
        # app.logger.error("No Supabase client for customer lookup") # Can't use app.logger here
    ...
    try:
        ...
        if customer_phone:
            ...
            if existing_customer:
                ...
                if not update_response.data:
                    print(f"ERROR: Failed to update customer {customer_id}: {update_response.data}")
                    # Continue attempting to return ID even if update fails
                else:
                    print(f"INFO: Updated customer: {customer_id}")
                    # No need to call sync_to_supabase_immediately here, as the update was direct
        ...
        if insert_response.data:
            ...
            # No need to call sync_to_supabase_immediately here, as the insert was direct
            return customer_id
        else:
            print(f"ERROR: Failed to create customer: {insert_response.data}")
            return None
...
def get_users_data():
    """Load users from Supabase first, fallback to JSON"""
    supabase_data = get_supabase_data('users')
    if supabase_data is not None:
        # Filter out super_admin users
        return [user for user in supabase_data if user.get('role') != 'super_admin']
