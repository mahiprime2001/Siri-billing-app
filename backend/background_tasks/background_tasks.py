            ...
            if not os.path.exists(file_path):
                ...
                try:
                    ...
                    if result['success'] and table_name in result['data'] and result['data'][table_name]:
                        ...
                        if table_name == 'Users':
                            records = [user for user in records if user.get('role') not in ['admin', 'super_admin']]
                            app.logger.info(f"Filtered out admin/super_admin users. {len(records)} users remaining.")
                ...
                for table_name, file_path in TABLE_FILE_MAP.items():
                    ...
                    if result['success']:
                        ...
                        if pulled_records:
                            ...
                            # ✅ Filter out admin AND super_admin users before merging
                            if table_name == 'Users':
                                pulled_records = [
                                    user for user in pulled_records
                                    if user.get('role') not in ['admin', 'super_admin']
                                ]
                                app.logger.info(f"Filtered out admin/super_admin users. {len(pulled_records)} users remaining.")
...
def start_background_tasks(app: Flask):
    ...
    offline_transfer_verification_queue_thread = threading.Thread(
        target=background_offline_transfer_verification_queue_scheduler, daemon=True, args=(app, 1,)
    )
...
def background_offline_transfer_verification_queue_scheduler(app: Flask, interval_minutes=1):
    """Periodically retry offline transfer-verification operations saved to local queue."""
    with app.app_context():
        app.logger.info("Starting offline transfer-verification queue scheduler.")
        while True:
            try:
